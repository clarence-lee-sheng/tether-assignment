...
# lighteval vllm "model_name=/home/ubuntu/tether-assessment/checkpoints/smollm2_135m_hf_trainer/checkpoint-2600,dtype=float16,max_model_length=2048,gpu_memory_utilization=0.8,generation_parameters={max_new_tokens:2048,temperature:0.6,top_p:0.95}" \
#     "custom|mmlu_cf|0|1" \
#     --output-dir evals/ \
#     --custom-tasks /home/ubuntu/tether-assessment/src/tether/eval/tasks.py \
#     --save-details
# """


# lighteval vllm "model_name=/home/ubuntu/tether-assessment/exports/smollm2_cosmo_test_2,dtype=float16,max_model_length=2048,gpu_memory_utilization=0.8,generation_parameters={max_new_tokens:2048,temperature:0.6,top_p:0.95}"     "custom|mmlu_cf|0|1"     --output-dir evals/     --custom-tasks /home/ubuntu/tether-assessment/src/tether/eval/tasks.py     --save-details
# """
...
lighteval vllm "model_name=/Users/grace/Documents/clarence/Research/tether_assessment/models/SmolLM2-135M,dtype=bfloat16,max_model_length=2048,gpu_memory_utilization=0.8,generation_parameters={max_new_tokens:8192,temperature:0.6,top_p:0.95}" \
    "custom|mmlu_cf|0|1" \
    --output-dir evals/ \
    --custom-tasks /Users/grace/Documents/clarence/Research/tether_assessment/src/tether/eval/tasks.py/tasks.py \
    --save-details
"""
...
class JudgeLLMMixEval(JudgeLLM):
    def compute(
        self, sample_ids: list[str], responses: list, formatted_docs: list[Doc], **kwargs
    ) -> dict[str, float]:
        """
        ...
        for response in responses:
            prediction_text = remove_reasoning_tags(
                response[0].result[0], tag_pairs=REASONING_TAG_PAIRS
            ).strip()
        ...
        print(
            f"Number of predictions truncated to fit within {MAX_INPUT_TOKENS} tokens: {num_truncated}"
        )  # noqa: T201
...
for task in TASKS_TABLE:
    task.metric = [
        metric for metric in task.metric if metric.category != MetricCategory.MULTICHOICE_PMI
    ]
